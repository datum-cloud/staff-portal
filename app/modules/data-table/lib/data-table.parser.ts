...
export const getFiltersStateParser = <TData>(columnIds?: string[] | Set<string>) => {
  ...
  return createParser({
    ...
    eq: (a, b) =>
      ...
      a.every(
        (filter, index) =>
          (filter as unknown as FilterItemSchema).id ===
            (b[index] as unknown as FilterItemSchema)?.id &&
          (filter as unknown as FilterItemSchema).value ===
            (b[index] as unknown as FilterItemSchema)?.value &&
          (filter as unknown as FilterItemSchema).variant ===
            (b[index] as unknown as FilterItemSchema)?.variant &&
          (filter as unknown as FilterItemSchema).operator ===
            (b[index] as unknown as FilterItemSchema)?.operator
      ),
  });
};
