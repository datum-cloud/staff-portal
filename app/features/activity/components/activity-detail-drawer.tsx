...
export default function ActivityDetailDrawer({
  ...
  useEffect(() => {
    if (!open) {
      setExpandedSections({});
      setShowRequestManagedFields(false);
      setShowResponseManagedFields(false);
  ...
  // Only filter out the managedFields field itself
  const MANAGED_FIELDS_KEYWORDS = ['managedFields'];
  ...
  const filterManagedFields = (data: any): any => {
    ...
    for (const [key, value] of Object.entries(data)) {
      if (key === 'managedFields') {
        // Skip the managedFields field entirely
        continue;
      }
      // Recursively filter nested objects
      filtered[key] = typeof value === 'object' ? filterManagedFields(value) : value;
    }
    return filtered;
  };
  ...
  const renderExpandableJson = (
    ...
    // Check if the original data contains any managed field keywords
    const originalDataStr = JSON.stringify(data);
    const hasManagedFields = MANAGED_FIELDS_KEYWORDS.some((keyword) =>
      originalDataStr.includes(`"${keyword}"`)
    );

    const managedFieldsTooltip = (
      <div className="max-w-xs space-y-2 text-sm">
        <p>
          <Trans>
            Tracks which field manager controls each field. Prevents overwrites and manages
            conflicts when multiple actors change fields.
          </Trans>
        </p>
        <p className="text-xs text-gray-400">
          <Trans>
            Automatically managed by the API server. Disable to hide managedFields and focus on
            other metadata.
          </Trans>
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200">
        ...
        {isExpanded && (
          <div className="flex flex-col bg-gray-900 text-gray-100">
            {/* Toolbar */}
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-700 px-3 py-2">
              <button
                onClick={() => handleCopy(jsonString, sectionId)}
                className="flex items-center gap-1.5 rounded bg-blue-600 px-2.5 py-1.5 text-xs font-medium text-white transition-colors hover:bg-blue-700"
                title={t`Copy to clipboard`}
                type="button">
                <Copy size={14} />
              ...
              {hasManagedFields && (
                <Tooltip message={managedFieldsTooltip}>
                  <label className="flex items-center gap-2 rounded px-2 py-1.5 transition-colors hover:bg-gray-800">
                    <input
                      type="checkbox"
                    ...
                    <span className="text-xs font-medium text-gray-300">
                      <Trans>Show managed fields</Trans>
                    </span>
                  </label>
                </Tooltip>
              )}
          ...
          <div>
            ...
            {entry.responseStatus ? (
              <div className="flex items-center gap-2">
                {entry.statusMessage ? (
                  <Tooltip
                    message={`${entry.responseStatus.code} ${
                      getDetailedStatusDescription(entry.responseStatus.code) || ''
                    }`}>
                    <BadgeState state={entry.category || 'info'} message={entry.statusMessage} />
                  </Tooltip>
                ) : (
                  <BadgeState
                ...
                {entry.responseStatus.reason && (
                  <span className="text-xs text-gray-600">{entry.responseStatus.reason}</span>
                )}
              </div>
            ) : (
              <BadgeState state={entry.category || 'info'} message={entry.category || 'Info'} />
            )}
          </div>
        </div>
      </CardContent>
    ...
    renderSection(
      ...
      <div className="space-y-2">
        ...
        {entry.user.groups && entry.user.groups.length > 0 && (
          <div>
            <Text textColor="muted" className="mb-1 text-xs font-bold tracking-wider uppercase">
              <Trans>Groups</Trans>
            </Text>
  ...
  const renderPairedRow = (
    ...
    return (
      <div className="grid gap-3 md:grid-cols-2">
        <div className={left ? 'w-full' : 'invisible'} aria-hidden={!left}>
          {left || <div className="h-full w-full" />}
        </div>
        <div className={right ? 'w-full' : 'invisible'} aria-hidden={!right}>
          {right || <div className="h-full w-full" />}
        </div>
      </div>
    );
  };
          ...
          {entry.auditId && (
            <div className="mt-3 flex items-center gap-2">
              <Text
                textColor="muted"
                className="flex-shrink-0 text-xs font-bold tracking-wider uppercase">
                <Trans>Audit ID</Trans>
              </Text>
              <p className="font-mono text-sm break-all text-gray-700">{entry.auditId}</p>
              <button
                onClick={() => entry.auditId && handleCopy(entry.auditId, 'auditId')}
                className="flex-shrink-0 rounded p-1 transition-colors hover:bg-gray-100"
                title={t`Copy to clipboard`}
                type="button">
                <Copy
                  size={14}
                  className={copiedField === 'auditId' ? 'text-green-600' : 'text-gray-600'}
                />
              </button>
            </div>
          )}
        ...
        <div className="flex-1 overflow-y-auto">
          {activeTab === 'overview' ? (
            <div className="space-y-3 px-6 py-4">
              {/* Row 1: Event Summary - Full Width */}
              <div className="lg:col-span-2">
                ...
                <Card className="border-2 border-blue-200 bg-blue-50 shadow-sm transition-shadow hover:shadow-md">
                  ...
                  <CardContent className="space-y-3 pt-2">
                    ...
                    {(() => {
                      const projectName =
                        entry.annotations?.['resourcemanager.miloapis.com/project-name'];
                      const organizationName =
                        entry.annotations?.['resourcemanager.miloapis.com/organization-name'];
                      const controlPlaneType =
                        entry.annotations?.['telemetry.miloapis.com/control-plane-type'];

                      const hasContext = projectName || organizationName || controlPlaneType;

                      if (!hasContext) return null;

                      const showPlatform = !projectName && !organizationName && controlPlaneType;
                      ...
                      return (
                        <div className="border-t border-blue-200 pt-2">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {projectName && (
                              <span className="rounded border border-blue-200 bg-blue-50 px-2 py-1 text-xs text-blue-900">
                            ...
                            {organizationName && (
                              <span className="rounded border border-purple-200 bg-purple-50 px-2 py-1 text-xs text-purple-900">
                                <span className="font-semibold">
                            ...
                            {showPlatform && controlPlaneType === 'core' && (
                            ...
                            {showPlatform && controlPlaneType === 'project' && (
                              <span className="rounded border border-blue-200 bg-blue-50 px-2 py-1 text-xs text-blue-900">
                                <span className="font-semibold">
                                  <Trans>Platform:</Trans>
                                </span>{' '}
              ...
              {/* Row 2: Resource & User - Split 50/50 */}
              <div className="grid gap-3 md:grid-cols-2">
              ...
              {/* Row 3: Request & Response - Split 50/50 */}
              <div className="grid gap-3 md:grid-cols-2">
                {/* Request Information */}
                {(entry.requestUri || entry.userAgent || entry.sourceIPs) &&
                  renderSection(
              ...
              {/* Row 4: Audit Metadata - Full Width */}
              {(entry.auditId ||
                entry.stage ||
                (entry.annotations && Object.keys(entry.annotations).length > 0)) &&
                renderSection(
