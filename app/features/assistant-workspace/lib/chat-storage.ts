...
/**
 * Local chat history. Intentionally shares the same storage key as the
 * slide-up assistant (`app/features/assistant`) so a user's chats show up on
 * both surfaces. The extra `model`/`effort` fields are optional and backward
 * compatible with chats saved by the older panel.
 */
