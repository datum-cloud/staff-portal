...
const defaultWebSocket = {
  upgradeWebSocket: () => Promise.resolve(new Response('Not Found', { status: 404 })),
  injectWebSocket: (server: any) => server,
...
export async function createWebSocket({ app, enabled }: Config): Promise<WebSocket> {
  ...
  const runtime = import.meta.env.REACT_ROUTER_HONO_SERVER_RUNTIME as 'node' | 'bun';
